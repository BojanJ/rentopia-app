...
import type { Booking, BookingsResponse, BookingsQueryParams } from '../types/booking';
...
export class BookingService {
   ...
   */
  static async getBookings(params?: BookingsQueryParams): Promise<BookingsResponse> {
  ...
  static async getBookingsForProperty(
    ...
    console.log("🏠 Getting bookings for property:", { propertyId, startDate, endDate });
  ...
  static async getBookingsForMonth(
    ...
    try {
      ...
      const response = await this.getBookingsForProperty(propertyId, startDate, endDate);
  ...
  static async updateBooking(id: string, bookingData: Partial<Booking>): Promise<Booking> {
    const response = await api.put<Booking>(`${this.BASE_URL}/${id}`, bookingData);
    return response.data;
  }
